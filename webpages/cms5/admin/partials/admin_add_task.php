...
if (isset($_POST['create_task']))
	...
	$error = ['title_error' => '', 'task_category_error' => '', 'task_desc_error' => '', 'tag_one_error' => '', 'tag_two_error' => '', 'tag_three_error' => '', 'tag_four_error' => '', 'word_count_error' => '', 'page_count_error' => '', 'file_format_error' => '', 'sample_error' => ''];
   ...
   <form action="" method = "post" enctype="multipart/form-data">
      <div class="form-group">
         ...
         <input type="text" class="form-control" name="title" autocomplete="on" 
            value="<?php
               echo isset($task_title) ? $task_title : '' ?>"
            >
         <p><?php
            echo isset($error['title_error']) ? $error['title_error'] : '' ?></p>
      </div>
      <div class="form-group">
         ...
         <p><?php
            echo isset($error['task_category_error']) ? $error['task_category_error'] : '' ?></p>
      </div>
      <div class="form-group">
         ...
         <p><?php
            echo isset($error['task_desc_error']) ? $error['task_desc_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="sel1"> Tags</label>
         <select class="form-control  task_tags" name="tag_one" attr_tag='1' placeholder="Major Subject">
            <option value="">Select</option>
            <?php
               ...
               while ($row = mysqli_fetch_assoc($select_all_tags_sidebar))
               	...
               	$tag_title = $row['tag'];
               ?>
            <option value="<?php echo $tag_title ?>"> <?php echo $tag_title ?> </option>
            <?php
               } ?>
         </select>
         <p><?php
            echo isset($error['tag_one_error']) ? $error['tag_one_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="sel1"> Tags</label>
         <select class="form-control task_tags" name="tag_two" attr_tag='2' placeholder="Major Subject" disabled = "disabled">
            
         </select>
         <p><?php
            echo isset($error['tag_two_error']) ? $error['tag_two_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="sel1"> Tags</label>
         <select class="form-control task_tags" name="tag_three" attr_tag='3' placeholder="Major Subject" disabled = "disabled">
            
         </select>
         <p><?php
            echo isset($error['tag_three_error']) ? $error['tag_three_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="sel1"> Tags</label>
         <select class="form-control task_tags" name="tag_four" attr_tag='4' placeholder="Major Subject" disabled = "disabled">
            
         </select>
         <p><?php
            echo isset($error['tag_four_error']) ? $error['tag_four_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="title">Word Count</label>
         <input type="text" class="form-control" name="word_count" autocomplete="on" 
            value="<?php
               echo isset($word_count) ? $word_count : '' ?>">
         <p><?php
            echo isset($error['word_count_error']) ? $error['word_count_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="title">Page Count</label>
         <input type="text" class="form-control" name="page_count" autocomplete="on" 
            value="<?php
               echo isset($page_count) ? $page_count : '' ?>">
         <p><?php
            echo isset($error['page_count_error']) ? $error['page_count_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="title">File Format</label>
         <input type="text" class="form-control" name="file_format" value="<?php
            echo isset($file_format) ? $file_format : '' ?>">
         <p><?php
            echo isset($error['file_format_error']) ? $error['file_format_error'] : '' ?></p>
      </div>
      <div class="form-group">
         <label for="post_doc">Sample</label>
         <input type ="file" class ="form-control" name ="sample">
         <p><?php
            echo isset($error['sample_error']) ? $error['sample_error'] : '' ?></p>
      </div>
      <div class="form-group">
...
$(".task_tags").change(function () { 
        ...
        //console.log(selected_tags);
			$.ajax({
				...
				dataType: 'HTML',
				url: "manageTags.php",		
				data: ({"tags_attr":$(this).attr('attr_tag'), 'selectedTag':getAllSelected()}),
				success: function(data){
					var b = JSON.parse(data);
					console.log(b.tr);
					console.log(b.disableArray);
					console.log(b.disableArray[1]);
					$("[attr_tag='"+b.enable+"']").prop( "disabled", false );
					$("[attr_tag='"+b.enable+"']").html('');
					$("[attr_tag='"+b.enable+"']").append(b.tr);
					
					},
				error:function(a,e,o){
					console.log("record is not found." + o);
				}
			});
	});
});
